...
# RANGO FECHA FINAL (inicia vacío)
fecha_final_rango = None
if "FECHA FINAL" in df.columns:
    st.sidebar.markdown("### Rango de fecha finalización")
    fecha_final_rango = st.sidebar.date_input("Selecciona rango", value=None)
...
# ACTOR PRIMER NIVEL eliminado del sidebar (se filtra fijo a 'internacional')
...
if boton_buscar:
    ...
    # ---------------------------------
    # FILTRO FIJO PERMANENTE
    # ACTOR PRIMER NIVEL = "internacional"
    # ---------------------------------
    if "ACTOR PRIMER NIVEL" in df_filtrado.columns:
        df_filtrado = df_filtrado[
            df_filtrado["ACTOR PRIMER NIVEL"].astype(str).str.lower().str.strip() == "internacional"
        ]
